...
import { browserFocalPositions, projectEditorInputSchema, type HorizontalFrame, type ProjectEditorInput } from "@shared/schemas/project-editor";
...
import { getSpecialBespokeLabel } from "@shared/lib/special-bespoke-modules";
...
function labelWithCue(label: string) {
...
function Field({ label, name, description, type = "text", required }: { label: string; name: string; description?: string; type?: string; required?: boolean }) {
  ...
  return <div className="field-group"><label>{labelWithCue(label)}{required ? " *" : ""}</label><input type={type} aria-invalid={Boolean(fieldError)} {...register(path(name), type === "number" ? { valueAsNumber: true } : undefined)} />{description && <p className="field-hint">{description}</p>}{fieldError && <p className="field-error">{fieldError}</p>}</div>;
}
...
function LineEditor({ label, name, fixedCount, max = 8, lineCues = {} }: { label: string; name: string; fixedCount?: number; max?: number; lineCues?: Record<number, string> }) {
  ...
  return <Controller control={control} name={path(name)} render={({ field }) => {
    ...
    return <fieldset className="line-editor"><legend>{label}</legend><div className="line-editor-list">{lines.map((value, index) => <div className="line-editor-row" key={index}><label htmlFor={`${name}-${index}`}>Line {index + 1}{lineCues[index] && <span className="authoring-cue"> ({lineCues[index]})</span>}</label><input id={`${name}-${index}`} value={value} onBlur={field.onBlur} onChange={(event) => { const next = [...lines]; next[index] = event.target.value; update(next); }} />{!fixedCount && <div className="line-editor-actions"><button type="button" aria-label={`Move line ${index + 1} up`} disabled={index === 0} onClick={() => move(index, -1)}>↑</button><button type="button" aria-label={`Move line ${index + 1} down`} disabled={index === lines.length - 1} onClick={() => move(index, 1)}>↓</button><button type="button" onClick={() => update(lines.filter((_, itemIndex) => itemIndex !== index))}>Remove</button></div>}</div>)}</div>{!fixedCount && lines.length < max && <button className="line-add-button" type="button" onClick={() => update([...lines, ""])}>+ Add line</button>}</fieldset>;
  }} />;
}

function AnimatedHeadingComposer({ paths, title = "Heading" }: { paths: { line1: string; line2First: string; line2Second: string; line3: string; line4First: string; line4Second: string }; title?: string }) {
  return <fieldset className="heading-composer"><legend>{title}</legend><Field label="Line 1" name={paths.line1} /><div className="heading-line-group"><p className="heading-line-label">Line 2</p><div className="phrase-row"><Field label="First phrase" name={paths.line2First} /><Field label="Second phrase (Animated)" name={paths.line2Second} /></div></div><Field label="Line 3 (Shifts right)" name={paths.line3} /><div className="heading-line-group"><p className="heading-line-label">Line 4</p><div className="phrase-row"><Field label="First phrase" name={paths.line4First} /><Field label="Second phrase (Shifts right)" name={paths.line4Second} /></div></div></fieldset>;
}
...
function TemplateFourHorizontalStory({ base }: { base: string }) {
  ...
  return <div className="horizontal-frame-list">{frames.map((frame, index) => {
    ...
    if (frame.type === "editorial") return <Scene number={index + 1} title="Editorial text" hint="Position fixed by the design" key={frame.id ?? `${frame.type}-${index}`}><LineEditor label="Opening heading" name={`${frameBase}.headingLines`} lineCues={{ 2: "Shifts right" }} /><TextArea label="Body copy" name={`${frameBase}.body`} /><AnimatedHeadingComposer paths={headingPaths} /></Scene>;
...
function IntroFields({ base, fixedHeading = false, lastLineShifts = false }: { base: string; fixedHeading?: boolean; lastLineShifts?: boolean }) { return <><LineEditor label="Heading" name={`${base}.headingLines`} fixedCount={fixedHeading ? 3 : undefined} lineCues={lastLineShifts ? { 2: "Shifts right" } : undefined} /><TextArea label="Body copy" name={`${base}.bodyCopy`} /></>; }
...
function DrawingTwelve({ base, showAccent = true }: { base: string; showAccent?: boolean }) { return <><div className="compact-grid"><Field label="Area label" name={`${base}.drawingAreaLabel`} /><Field label="Title" name={`${base}.drawingTitle`} />{showAccent && <Field label="Accent color" name={`${base}.accentColor`} />}</div><TextArea label="Description" name={`${base}.drawingDescription`} /><MediaField label="Drawing media" base={`${base}.media`} /></>; }
...
function TwoImageNarrative({ base, defaults }: { base: string; defaults: readonly [{ depth: NarrativeImage["depth"]; speed: number }, { depth: NarrativeImage["depth"]; speed: number }] }) {
  const { setValue } = useFormContext<ProjectEditorInput>();
...
function TemplateContent({ template }: { template: TemplateType }) {
  ...
  const templateOneBespokeModule = useWatch<ProjectEditorInput>({
    name: path(`${base}.bespoke.module`),
  });
  const specialBespokeLabel = getSpecialBespokeLabel(templateOneBespokeModule);
  const sectionNames = template === "template-1" ? ["Statement", "Story", specialBespokeLabel ? "Special Project Animation" : "Custom Scroll Sequence", "Feature", "Horizontal Story", "Drawing"] : template === "template-2" ? ["Intro", "Horizontal Story", "Narrative", "Drawing"] : template === "template-3" ? ["Intro", "Scroll Sequence", "Horizontal Story", "Drawing"] : ["Intro", "Horizontal Story", "Narrative", "Drawing"];
  ...
  if (template === "template-3") return <>
    ...
    <SequenceSection base={`${base}.bespoke`} label="Scroll Sequence" />
  ...
  return <>{sectionNav}<SectionToggle label="Intro" description="Opening heading and project introduction." name={`${base}.intro.enabled`}><IntroFields base={`${base}.intro`} /></SectionToggle><SectionToggle label="Horizontal Story" description="A fixed sequence of editorial and image scenes." name={`${base}.horizontalStory.enabled`}><p className="section-intro">Scene order and presentation are set by the website design. Edit only the content and media below.</p><TemplateFourHorizontalStory base={`${base}.horizontalStory`} /></SectionToggle><SectionToggle label="Narrative" description="Editorial copy accompanied by layered imagery." name={`${base}.narrative.enabled`}><div className="narrative-media-group"><TwoImageNarrative base={`${base}.narrative`} defaults={[{ depth: "rear", speed: 0.85 }, { depth: "rear", speed: 1.05 }]} /></div><div className="takeover-media-group"><p className="eyebrow">Takeover image</p><MediaField label="Takeover image — optional" base={`${base}.narrative.takeoverImage`} optional /></div></SectionToggle><SectionToggle label="Drawing" description="Project drawing and its supporting description. Its color follows the project Theme." name={`${base}.drawing.enabled`}><DrawingTwelve base={`${base}.drawing`} showAccent={false} /></SectionToggle></>;
}

function SpecialBespokeSection({ base, label }: { base: string; label: string }) {
  return <SectionToggle label="Special Project Animation" description="Existing Studio K2 animation. Its renderer assignment is preserved automatically." name={`${base}.enabled`}><div className="derived-value"><span>Assigned animation</span><strong>{label}</strong></div></SectionToggle>;
}

function SequenceSection({ base, label, fixedModule }: { base: string; label: string; fixedModule?: "scroll-video" }) {
  const { setValue } = useFormContext<ProjectEditorInput>();
  const frameCount = Number(useWatch<ProjectEditorInput>({ name: path(`${base}.frameCount`) }) ?? 0);
  const [busy, setBusy] = useState(false); const [message, setMessage] = useState("");
  async function upload(files: FileList | null) { if (!files?.length) return; setBusy(true); setMessage(""); try { const result = await uploadSequence(Array.from(files)); if (fixedModule) setValue(path(`${base}.module`), fixedModule as never, { shouldDirty: true }); setValue(path(`${base}.framePath`), result.framePath as never, { shouldDirty: true }); setValue(path(`${base}.frameCount`), result.frameCount as never, { shouldDirty: true }); setMessage(`${result.frameCount} contiguous frames validated.`); } catch (reason) { setMessage(reason instanceof Error ? reason.message : "Sequence upload failed"); } finally { setBusy(false); } }
  return <SectionToggle label={label} description="The established image sequence used in this project." name={`${base}.enabled`}><div className="collection-toolbar"><div><strong>Sequence frames</strong><p>Upload a complete, consecutively numbered sequence.</p></div><label className="upload-button">{busy ? "Validating frames…" : frameCount ? "Replace sequence" : "Upload sequence"}<input type="file" multiple accept="image/jpeg,image/png,image/webp" disabled={busy} onChange={(event) => void upload(event.target.files)} /></label></div><div className="derived-value"><span>Derived frame count</span><strong>{frameCount}</strong></div>{message && <p className="field-hint">{message}</p>}</SectionToggle>;
}

function GalleryEditor({ showTitle = true, guidedVisibleCount = false }: { showTitle?: boolean; guidedVisibleCount?: boolean }) {
  ...
  const [visibleCount, setVisibleCount] = useState<3 | 6 | 9>(() => images.length === 6 ? 6 : images.length === 9 ? 9 : 3);
  const visibleLimit = guidedVisibleCount ? visibleCount : 9;
  async function add(files: FileList | null, expandedOnly = false) { if (!files?.length) return; const uploaded = await uploadImages(Array.from(files)); const additions = uploaded.map((file) => ({ id: crypto.randomUUID(), src: file.url, alt: "", caption: "", focalPosition: "center" })); if (expandedOnly) setValue("galleryConfig.expandedImages", [...(expanded ?? images), ...additions], { shouldDirty: true, shouldValidate: true }); else { const next = [...images, ...additions].slice(0, visibleLimit); setValue("galleryConfig.images", next, { shouldDirty: true, shouldValidate: true }); if (expanded) { const included = additions.slice(0, Math.max(0, visibleLimit - images.length)); setValue("galleryConfig.expandedImages", [...expanded, ...included], { shouldDirty: true, shouldValidate: true }); } } }
  ...
  function chooseVisibleCount(count: 3 | 6 | 9) { setVisibleCount(count); if (images.length > count) update(images.slice(0, count)); }
  ...
  return <>
    ...
    {guidedVisibleCount && <fieldset className="choice-fieldset gallery-count"><legend>Visible gallery</legend><p className="field-hint">Display</p><div className="choice-row three-up">{([3, 6, 9] as const).map((count) => <label className="choice-option" key={count}><input type="radio" name="visible-gallery-count" checked={visibleCount === count} onChange={() => chooseVisibleCount(count)} /><span>{count} Images</span></label>)}</div></fieldset>}
    <div className="collection-toolbar"><div><strong>Visible gallery images</strong><p>{guidedVisibleCount ? `${images.length} of ${visibleCount} selected` : `${images.length} selected · enabled galleries require 3, 6, or 9`}</p></div><label className={`upload-button ${images.length >= visibleLimit ? "is-disabled" : ""}`}>Upload images<input type="file" multiple accept="image/jpeg,image/png,image/webp" disabled={images.length >= visibleLimit} onChange={(event) => void add(event.target.files)} /></label></div>
...
export function ProjectEditorForm({ initial, onSave, saving, serverError }: { initial: ProjectEditorData; onSave: (input: ProjectEditorInput) => Promise<void>; saving: boolean; serverError?: string }) {
  ...
  async function submit(input: ProjectEditorInput) {
    ...
    if (input.templateConfig.template === "template-3") {
      ...
      const { frame1Image, frame2Image, frame3Image, frame4Image, ...horizontalContent } = horizontalStory;
      next = { ...next, templateConfig: { ...input.templateConfig, sections: { ...input.templateConfig.sections,
        horizontalStory: {
          ...horizontalContent,
          ...(frame1Image ? { frame1Image } : {}),
          ...(frame2Image ? { frame2Image } : {}),
          ...(frame3Image ? { frame3Image } : {}),
          ...(frame4Image ? { frame4Image } : {}),
          accentColor: input.themeConfig.horizontalBackgroundColor,
          textColor: input.themeConfig.horizontalTextColor,
        },
        drawing: { ...drawing, accentColor: input.themeConfig.horizontalBackgroundColor },
      } } };
    }
    if (input.templateConfig.template === "template-4") {
      const drawing = input.templateConfig.sections.drawing;
      next = { ...next, templateConfig: { ...input.templateConfig, sections: { ...input.templateConfig.sections,
        drawing: { ...drawing, accentColor: input.themeConfig.horizontalBackgroundColor },
      } } };
  ...
  <section className="editor-panel" hidden={active !== "Gallery"}><p className="eyebrow">Gallery</p><h2>Project gallery</h2><GalleryEditor showTitle={template !== "template-1" && template !== "template-2"} guidedVisibleCount={template === "template-1"} /></section>
